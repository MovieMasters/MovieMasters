...
public class MovieDAO extends DAO{
    ...
    public MovieCollection getActualMovies(){
        ...
        String query =    "SELECT movie.id, movie.title, movie.releaseDate, movie.playTime, movie.summary, `language`.language FROM " +
                                "movie INNER JOIN (`language`, `show`) ON (`language`.code = movie.languageCode AND `show`.movieId = movie.id) WHERE cast( concat( date_format(show.date, '%Y-%m-%d '), time_format(show.time, '%H:%i:%s') ) as datetime) >= now()";
    ...
    public HashMap<String, ArrayList<Show>> getShows(int movieId){
        ...
        String query = "SELECT `show`.id, `show`.date, `show`.time, `show`.roomId, `theater`.name as `theater_name` FROM `show` INNER JOIN `room` ON `show`.roomId = `room`.id INNER JOIN `theater` on `room`.theaterName = `theater`.name WHERE `show`.movieId = ? AND `show`.date >= CURDATE() AND cast( concat( date_format(show.date, '%Y-%m-%d '), time_format(show.time, '%H:%i:%s') ) as datetime) >= now() ORDER BY `theater`.name";
        Connection conn = DBConnection.getConnection();
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, movieId);
            ResultSet rs = stmt.executeQuery();
